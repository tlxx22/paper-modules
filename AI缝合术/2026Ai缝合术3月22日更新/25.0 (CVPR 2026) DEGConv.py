...
BinaryTensor = NewType('BinaryTensor', torch.Tensor)  # A type where each element is in {-1, 1}                                                                                                                           # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
...
class STESign(Function):
    ...
    Straight-Through Estimator (STE) is used to approximate the gradient of sign function.                                                                                                                           # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
    """
...
class BinaryAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., attn_quant=False, attn_bias=False, pv_quant=False, input_size=None):                                                                                                                           # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
        super().__init__()
        ...
        if self.attn_bias: # dense bias
            ...
            relative_position_index = \
                torch.zeros(size=(input_size[0] * input_size[1] + 1, ) * 2, dtype=relative_coords.dtype)                                                                                                                           # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
    ...
    def forward(self, x):
        ...
        if self.attn_quant:
            ...
            if self.attn_bias:
                relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(                                                                                                                           # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
...
if __name__ == "__main__":
    ...
    model = BinaryAttention(dim=64, num_heads=8).to(device)
    print(model)
