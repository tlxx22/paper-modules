...
def _to_scalar(x):
    """将标量Tensor转换为Python标量；非Tensor原样返回。"""
    return x.item() if isinstance(x, torch.Tensor) else x

def _get_capturable_supported_devices(supports_xla: bool = False):
    """简化版：返回支持capturable的设备集合。"""
    # 够用即可；如需更严谨可依据版本扩展
    return {"cpu", "cuda", "mps"}

def _get_value(x):
    """取Python标量值（和 _to_scalar 类似，这里保持行为一致）。"""
    return x.item() if isinstance(x, torch.Tensor) else x

def _stack_if_compiling(seq):
    """编译态下有些实现会stack；这里保持向后兼容，非编译直接返回原序列。"""
    if torch.compiler.is_compiling():
        # 当seq是标量Tensor列表时再stack，避免类型不匹配
        if isinstance(seq, (list, tuple)) and len(seq) > 0 and isinstance(seq[0], torch.Tensor):                                                                                                                                                                     # 微信公众号:AI缝合术
            return torch.stack(list(seq))
    return seq

def _view_as_real(*args, **kwargs):
    """占位：你的路径不使用复杂张量时可为空实现。若要支持complex，请按需改写。"""
    return
...
def to_local(tensor: Union[Tensor, List[Tensor]]) -> Union[Tensor, List[Tensor]]:
    ...
    if isinstance(tensor, Tensor):
        return tensor.to_local() if isinstance(tensor, DTensor) else tensor                                                                                                                                                                     # 微信公众号:AI缝合术
...
def dtensor_from_local(
    ...
    return [
        DTensor.from_local(t, device_mesh=device_mesh, placements=placements)                                                                                                                                                                     # 微信公众号:AI缝合术
        for t in tensor
    ]
...
class AsyncRuntime:
    ...
    def __init__(
        self, task_gen: Generator["AsyncTask", None, None], max_concurrent_tasks: int                                                                                                                                                                     # 微信公众号:AI缝合术
    ):
    ...
    def run(self):
        ...
        while have_new_tasks or previous_tasks:
            ...
            running_tasks = []
            if have_new_tasks and len(previous_tasks) < self._max_concurrent_tasks:                                                                                                                                                                     # 微信公众号:AI缝合术
...
def _multi_tensor_c_adam(
    ...
    *,
    ...
    if not torch.compiler.is_compiling() and capturable:
        capturable_supported_devices = _get_capturable_supported_devices(supports_xla=False)                                                                                                                                                                     # 微信公众号:AI缝合术
        assert all(
            p.device.type == step.device.type
            and p.device.type in capturable_supported_devices
            for p, step in zip(params, state_steps)
        ), (
            f"If capturable=True, params and state_steps must be on supported devices: {capturable_supported_devices}."                                                                                                                                                                     # 微信公众号:AI缝合术
        )
    ...
    for (
        ...
        if has_complex:
            if amsgrad:
                ...
                _view_as_real(device_params, device_grads, device_exp_avgs, device_exp_avg_sqs, device_max_exp_avg_sqs)                                                                                                                                                                     # 微信公众号:AI缝合术
            else:
        ...
        if capturable:
            ...
            if cautious:
                ...
                mask = [m.gt(0.0).to(e.dtype) for m, e in zip(mask, device_exp_avgs)]                                                                                                                                                                     # 微信公众号:AI缝合术
                mean_mask = [m.mean().clamp(min=1e-3) for m in mask]
                mask = [m / mm for m, mm in zip(mask, mean_mask)]
                masked_exp_avg = torch._foreach_mul(device_exp_avgs, mask)
                ...
                torch._foreach_addcdiv_(
                    device_params, device_exp_avgs, exp_avg_sq_sqrt, step_size                                                                                                                                                                     # 微信公众号:AI缝合术
                )
...
def c_adamw_update_foreach_async(
    ...
    torch._foreach_lerp_(V, G_square, [1 - beta2] * batch_size)                                                                                                                                                                     # 微信公众号:AI缝合术
    ...
    mask = [m.gt(0.0).to(e.dtype) for m, e in zip(mask, M)]                                                                                                                                                                     # 微信公众号:AI缝合术
...
class AdamW(Optimizer):
    ...
    def _init_group(
        ...
        for p in group["params"]:
            if p.grad is not None:
                has_complex |= torch.is_complex(p)                                                                                                                                                                     # 微信公众号:AI缝合术
    ...
    def _create_tasks(self, param_groups):
        for group in param_groups:
            for params in create_param_batches(group["params"], batch_size = self._world_size):                                                                                                                                                                     # 微信公众号:AI缝合术
    ...
    def step(self, closure: Callable = None):
        ...
        if self._foreach:
            ...
            '''
            params_with_grad, grads, exp_avgs, exp_avg_sqs, state_steps = [], [], [], [], []                                                                                                                                                                     # 微信公众号:AI缝合术
            max_exp_avg_sqs = []
        ...
        for group in self.param_groups:
            for i, p in enumerate(group["params"]):
                ...
                if type(grad) is torch.distributed.tensor.DTensor:
                    ...
                    mask = distribute_tensor(mask, device_mesh = grad.device_mesh, placements = grad.placements)                                                                                                                                                                     # 微信公众号:AI缝合术
                else:
...
def test_c_adamw():
    ...
    before = copy.deepcopy([p.detach().clone() for p in model.parameters()])                                                                                                                                                                     # 微信公众号:AI缝合术
    ...
    after = [p.detach().clone() for p in model.parameters()]                                                                                                                                                                     # 微信公众号:AI缝合术
    ...
    for i, (b, a) in enumerate(zip(before, after)):
        ...
        print(f"\n参数变化: Param {i} changed by: {diff:.6f}\n")                                                                                                                                                                     # 微信公众号:AI缝合术
        assert diff > 0, f"Parameter {i} did not change!"

    print("✅ 成功运行C-AdamW进行参数优化! ")
