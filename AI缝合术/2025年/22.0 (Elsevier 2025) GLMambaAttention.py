...
class GLMambaAttention(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
        ...
        **kwargs,
        ...
        self.local_spatial_attention = LocalSpatialAttention(32,8)
    ...
    def forward(self, input: torch.Tensor):
        ...
        input_left = self.local_spatial_attention ( input_left)
...
if __name__ == "__main__":
    ...
    # 初始化
    model = GLMambaAttention(hidden_dim=C, d_state=16).cuda()
    print(model)
    # 运行模型前向传播
    output = model(x)
    print("\n微信公众号: AI缝合术!\n")
