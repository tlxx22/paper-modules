...
# 中文题目：自适应矩形卷积用于遥感图像融合
# 论文链接：https://arxiv.org/pdf/2503.00467
# 官方github：https://github.com/WangXueyang-uestc/ARConv
# 所属机构：电子科技大学
# 代码整理：微信公众号：AI缝合术
...
if __name__ == "__main__":
    ...
    # 模块参数
    batch_size = 1    # 批大小
    in_planes = 32    # 输入特征通道数
    height = 256      # 图像高度
    width = 256       # 图像宽度
    
    # 创建 ARConv 模块
    arconv = ARConv(inc=32, outc=32, kernel_size=3, padding=1, stride=1, l_max=9, w_max=9)
    print(arconv)
    print("微信公众号:AI缝合术, nb!")
    
    # 生成随机输入张量 (batch_size, in_planes, height, width)
    x = torch.randn(batch_size, in_planes, height, width)
    
    # 打印输入张量的形状
    print("Input shape:", x.shape)
    
    # 前向传播, epoch为训练总轮次; hw_range表示不同的高度和宽度学习范围,论文中表5有讨论
    output = arconv(x, epoch=1, hw_range=[1, 18])
