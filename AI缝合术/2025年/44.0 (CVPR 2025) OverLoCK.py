...
if __name__ == "__main__":
    ...
    # 定义输入张量，假设图像尺寸为 224x224
    input_tensor = torch.randn(1, 3, 224, 224).to(device)  # [batch_size, channels, height, width]
    print("Input shape:",input_tensor.shape)
    
    # 初始化 overlock_xt 模型
    model = model = overlock_xt(pretrained=False).to(device)

    model.eval()  # 设置为评估模式，避免返回 dict

    # 执行前向传播
    with torch.no_grad():  # 关闭梯度计算，加快推理速度
        output = model(input_tensor)

    print("ImageNet1000分类结果:")
