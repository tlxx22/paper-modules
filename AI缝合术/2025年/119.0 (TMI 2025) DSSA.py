...
def _grid2seq(x: Tensor, region_size: Tuple[int], num_heads: int):
    ...
    x = x.view(B, num_heads, C // num_heads, region_h, region_size[0], region_w, region_size[1])                                                                                                                      # 微信公众号:AI缝合术
    x = torch.einsum('bmdhpwq->bmhwpqd', x).flatten(2, 3).flatten(-3, -2)
    return x, region_h, region_w


def _seq2grid(x: Tensor, region_h: int, region_w: int, region_size: Tuple[int]):                                                                                                                      # 微信公众号:AI缝合术
    """
    Args: 
    ...
    x = torch.einsum('bmhwpqd->bmdhpwq', x).reshape(bs, nhead * head_dim,
                                                    region_h * region_size[0], region_w * region_size[1])                                                                                                                      # 微信公众号:AI缝合术
    return x
...
def dual_sparse_attention(
    ...
    value_g = torch.gather(value.view(bs, nhead, 1, kv_nregion, kv_region_size, head_dim). \
                           expand(-1, -1, query.size(2), -1, -1, -1), dim=3,
                           index=broadcasted_region_graph)  # (bs, nhead, q_nregion, topk, kv_region_size, head_dim)                                                                                                                      # 微信公众号:AI缝合术
    ...
    v_g_un = value_g.flatten(-3, -2).unsqueeze(-3).expand(-1, -1, -1, attn.size(3), -1, -1)# (bs, nhead, q_nregion, reg_size, topk*kv_region_size, head_dim)                                                                                                                      # 微信公众号:AI缝合术
...
class DSSA(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def forward(self, x: Tensor, ret_attn_mask=False):
        ...
        q_r = F.avg_pool2d(q.detach(), kernel_size=region_size, ceil_mode=True, count_include_pad=False)                                                                                                                      # 微信公众号:AI缝合术
        k_r = F.avg_pool2d(k.detach(), kernel_size=region_size, ceil_mode=True, count_include_pad=False)  # nchw                                                                                                                      # 微信公众号:AI缝合术
        ...
        output = output.permute(0, 1, 2, 3)  # ncHW->nHWc
        if ret_attn_mask:
...
if __name__ == "__main__":
    ...
    # 输入张量：形状为 (B, C, H, W)
    x = torch.randn(1, 32, 64, 64)  # 例如 batch=1, 通道=32, 高=64, 宽=64                                                                                                                      # 微信公众号:AI缝合术

    # 初始化 DSSA 模块
    dssa = DSSA(dim=32, num_heads=8, n_win=7, topk=4, side_dwconv=3)                                                                                                                      # 微信公众号:AI缝合术
