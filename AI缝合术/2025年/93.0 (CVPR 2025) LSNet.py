...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.dw(q)
        q, k, v = q.view(B, self.num_heads, -1, N), k.view(B, self.num_heads, -1, N), v.view(B, self.num_heads, -1, N)                 # 微信公众号:AI缝合术
        attn = (
...
class RepVGGDW(torch.nn.Module):
    ...
    def fuse(self):
        ...
        identity = torch.nn.functional.pad(torch.ones(conv1_w.shape[0], conv1_w.shape[1], 1, 1, device=conv1_w.device), [1,1,1,1])                # 微信公众号:AI缝合术
...
class LSNet(torch.nn.Module):
    def __init__(self, img_size=224,
        ...
        resolution = img_size
        self.patch_embed = torch.nn.Sequential(Conv2d_BN(in_chans, embed_dim[0] // 4, 3, 2, 1), torch.nn.ReLU(),                    # 微信公众号:AI缝合术
        ...
        if distillation:
            self.head_dist = BN_Linear(embed_dim[-1], num_classes) if num_classes > 0 else torch.nn.Identity()                        # 微信公众号:AI缝合术
...
if __name__ == "__main__":
    ...
    model = lsnet_t(num_classes=1000, distillation=False)
    ...
    print("输出ImageNet-1K分类结果:", output.shape)  # [B, num_classes]
