...
if __name__ == "__main__":
    ...
    # 创建模型
    model = FCB(input_chs=input_channels, num_rows=height, num_cols=width)

    # 配置设备
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)

    # 使用 torchsummary 的 summary 函数打印模型信息
    summary(model, input_size=(input_channels, height, width), device=str(device))
