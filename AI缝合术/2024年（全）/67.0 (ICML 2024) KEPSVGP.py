...
class KEP_SVGPAttention(nn.Module):
    def __init__(self, dim, num_heads=8, embed_len=64, low_rank=10, rank_multi=10, concate=False, \
        ...
        self.we = nn.Parameter(nn.init.orthogonal_(torch.Tensor(self.num_heads, min(self.embed_len, self.low_rank * self.rank_multi), self.low_rank)))                                                                                              # 微信公众号:AI缝合术
        self.wr = nn.Parameter(nn.init.orthogonal_(torch.Tensor(self.num_heads, min(self.embed_len, self.low_rank * self.rank_multi), self.low_rank)))                                                                                              # 微信公众号:AI缝合术
        ...
        self.s_sqrt_low_triangle = nn.Parameter(nn.init.normal_(torch.Tensor(1, self.num_heads, self.low_rank, self.low_rank, self.low_rank)))                                                                                              # 微信公众号:AI缝合术
    ...
    def forward(self, x):
        ...
        if self.concate:
            samples = mean + (v2.permute(0,1,3,2,4) @ torch.randn(B, self.num_heads, 2*N, mean.shape[3], 1).to(x.device)).squeeze()                                                                                              # 微信公众号:AI缝合术
        else:
            samples = mean + (v2.permute(0,1,3,2,4) @ torch.randn(B, self.num_heads, N, mean.shape[3], 1).to(x.device)).squeeze()                                                                                              # 微信公众号:AI缝合术
        ...
        kl += 0.5 * (mu_d.permute(0,1,2,4,3) @ (lambda_sqrt_inv_diag.unsqueeze(0).unsqueeze(2) ** 4) @ mu_d).sum()                                                                                              # 微信公众号:AI缝合术
...
if __name__ == "__main__":
    ...
    # 初始化model
    model = KEP_SVGPAttention(dim=32, num_heads=4, embed_len=height*width)                                                                                              # 微信公众号:AI缝合术
    ...
    print(model)
    print("\n微信公众号:AI缝合术\n")
    print("输入张量形状:", x.shape)      # [B, H * W, C]
    print("输出张量形状:", output.shape)  # [B, H * W, C]