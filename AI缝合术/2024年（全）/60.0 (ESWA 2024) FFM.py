...
if __name__ == "__main__":
    ...
    # 创建输入张量
    feat1 = torch.randn(batch_size, channels, height, width).cuda()  # 输入图像特征图1
    feat2 = torch.randn(batch_size, height * width, channels).cuda()  # 输入图像特征图2

    # 初始化 FFM 模块
    feature_fusion = FFM(dim1=channels, dim2=channels, res=height).cuda()
    print(feature_fusion)
    print("\n微信公众号: AI缝合术!\n")
    # 前向传播测试
    output = feature_fusion(feat1, feat2)

    # 打印输入和输出的形状
    print(f"Input feature1 shape:  {feat1.shape}")
    print(f"Input feature2 shape:  {feat2.shape}")
