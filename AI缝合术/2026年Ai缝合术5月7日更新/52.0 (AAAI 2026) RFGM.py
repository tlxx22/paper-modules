...
class ProcessBlock(nn.Module):
    def __init__(self, in_nc, spatial = True):
        ...
        self.spatial_process = SpaBlock(in_nc) if spatial else nn.Identity()                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
        self.frequency_process = LightTopKFreBlock(nc = in_nc, top_k=in_nc)
        self.cat = nn.Conv2d(2*in_nc,in_nc,1,1,0) if spatial else nn.Conv2d(in_nc,in_nc,1,1,0)                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
...
class LightTopKFreBlock(nn.Module):
    def __init__(self, nc, top_k):
        ...
        self.magGuideFusion = MagGuidedFusion(channels=nc)                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
...
class MagGuidedFusion(nn.Module):
    def __init__(self, channels):
        ...
        self.expand_conv = nn.Conv2d(1, channels, kernel_size=1, stride=1, padding=0)                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
    ...
    def forward(self, mag0, mag1):
        ...
        similarity_matrix = torch.bmm(mag0_norm, mag1_norm.transpose(1, 2))  # (B, C, C)                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
        similarity_scores = similarity_matrix.mean(dim=-1)  # (B, C)
        top1_indices = torch.argmax(similarity_scores, dim=-1)  # (B,)
        mag0_top1 = torch.stack([mag0[b, top1_indices[b]] for b in range(B)], dim=0).unsqueeze(1)  # (B, 1, H, W)                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
...
class RFGM(nn.Module):
    def __init__(self, in_dim, out_dim, n=8):
        super(RFGM,self).__init__()
        self.in_dim = in_dim
        nc = self.in_dim // n
        self.out_dim = out_dim
        self.conv0 = nn.Sequential(
            nn.Conv2d(self.in_dim, nc, 1, 1, 0),
            ProcessBlock(nc),
        )
        ...
        self.convout = nn.Sequential(
            ProcessBlock(nc * 2),
            nn.Conv2d(nc * 2, self.out_dim, 1, 1, 0),
        )
    ...
    def forward(self, x):
        ...
        xout = self.convout(torch.cat((x, x5), dim=1))                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
...
if __name__ == "__main__":
    ...
    model = RFGM(in_dim=32, out_dim=32, n=8).to(device)                                                                                                                                                                                            # 哔哩哔哩/微信公众号: A-I-缝-合-术, AI-Feng-he-shu, 缝-合-术-AI, AIf-eng-hes-hu独家整理!
