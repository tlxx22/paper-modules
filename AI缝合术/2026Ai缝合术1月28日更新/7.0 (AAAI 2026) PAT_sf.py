...
RPEIndexFunction = None
    ...
    # warnings.warn(RED_STR.format("[WARNING] The module `rpe_ops` is not built. For better training performance, please build `rpe_ops`."),)
...
def get_rpe_config(ratio=1.9,
    ...
    method: METHOD or str
        ...
        The `METHOD` class is defined in `irpe.py`.
    mode: str or None
        The mode of image relative position encoding.                                                                                                                              # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
        Choices: [None, 'bias', 'contextual']
    shared_head: bool
        Whether to share weight among different heads.
    skip: int
    ...
    config.rpe_q = get_single_rpe_config(**kwargs) if 'q' in rpe_on else None                                                                                                                              # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
...
def build_rpe(config, head_dim, num_heads):
    ...
    rpes = [config.rpe_q, config.rpe_k, config.rpe_v]                                                                                                                              # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
...
class PAT_sf(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0, rpe_config=None):
        ...
        # image relative position encoding
        self.rpe_q, self.rpe_k, self.rpe_v = build_rpe(rpe_config, head_dim=head_dim, num_heads=num_heads)                                                                                                                              # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
...
if __name__ == "__main__":
    ...
    # 创建一个随机输入特征图 
    input_tensor = torch.randn(2, 16, 32, 32).to(device)    # (batch_size, channels, height, width)

    # 构建PAT_sf模型
    model = PAT_sf(dim=16, num_heads=8, rpe_config=get_rpe_config()).to(device)
    print(model)
