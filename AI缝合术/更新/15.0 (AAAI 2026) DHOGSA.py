...
class DHOGSA(nn.Module):
    ...
    def reshape_attn(self, q, k, v, ifBox):
        ...
        q = rearrange(q, '{} -> {}'.format(shape_ori, shape_tar), factor=self.factor, hw=hw, head=self.num_heads)                                                                                                                             # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
        ...
        out = (attn @ v)
        out = rearrange(out, '{} -> {}'.format(shape_tar, shape_ori), factor=self.factor, hw=hw, b=b, head=self.num_heads)                                                                                                                             # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
    ...
    def merge_patches(self, patches, shape_info):
        ...
        patches = rearrange(patches, 'b (h w) c (p1 p2) -> b c (h p1) (w p2)', h=n_h, w=n_w, p1=self.patch_size, p2=self.patch_size)                                                                                                                             # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
        if pad_h > 0 or pad_w > 0:
            patches = patches[:, :, :h, :w]
        return patches
    ...
    def apply_hog_to_patch(self, x_half):
        ...
        orientation_bin = ((orientation + torch.pi) / (2 * torch.pi) * self.n_bins).long() % self.n_bins                                                                                                                             # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
    ...
    def forward(self, x):
        ...
        n_w = shape_info[-1]
        hog_map = rearrange(hog_features, 'b (nh nw) bins -> b bins nh nw', nh=n_h, nw=n_w).contiguous()                                                                                                                             # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
        ...
        patches_out = torch.scatter(patches_out, -1, idx_patch, patches_out)                                                                                                                             # 哔哩哔哩/微信公众号: AI缝合术, 独家整理!
...
if __name__ == "__main__":
    ...
    input_tensor = torch.randn(1, 16, 32, 32).to(device)
    model = DHOGSA(dim=16, num_heads=4, bias=False).to(device)
    print(model)
