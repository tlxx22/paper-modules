...
讲解视频：https://www.bilibili.com/video/BV1T5seefEAo/

        High-Low Frequency Decomposition Block  高低频分解块：
        1、高频：高频信息代表图像的局部细节，有助于恢复图像细节，有利于去模糊和去噪等任务。
        2、低频：低频信息代表图像的整体结构，有助于恢复图像背景和轮廓，有利于超分辨率重建和多帧HDR重建。
        3、分解：高低频信息对不同退化类型的适用性，对图像的高频和低频信息进行分离，分别选择不同特征提取方案。
"""
...
class CALayer(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        super(CALayer, self).__init__()  # 调用父类nn.Module的初始化方法
...
class HLFD(nn.Module):
    def __init__(self, dim, wave='haar'):
        ...
        self.dense=Dense(n_feats)       # 高频信息 细节 https://www.bilibili.com/video/BV1BetCemEyy/
        self.unet=UNet(n_feats, wave)   # 低频信息 轮廓 https://www.bilibili.com/video/BV1XisXexEXs/
    ...
    def forward(self, x):
        ...
        low = self.down(x)  # torch.Size([1, 32, 32, 32])

        # F.interpolate函数用于对张量进行上采样或下采样 ：low: 这是你想要进行插值操作的输入张量、size=x.size()[-2:]: 输出张量的空间维度大小（即高度和宽度）
        up = F.interpolate(low, size=x.size()[-2:], mode='bilinear', align_corners=True)

        high = x - up  # torch.Size([1, 32, 64, 64])

        lowf = self.unet(low)  # torch.Size([1, 32, 32, 32])   低频信息 轮廓

        highfeat = self.dense(high)  # torch.Size([1, 32, 64, 64])   高频信息 细节

        lowfeat = F.interpolate(lowf, size=x.size()[-2:], mode='bilinear', align_corners=True)  # torch.Size([1, 32, 64, 64])

        temp = self.alise1(torch.cat([highfeat, lowfeat], dim=1))   # torch.Size([1, 32, 64, 64])

        temp = self.att(temp)       # torch.Size([1, 32, 64, 64])

        out=self.alise2(temp) + x       # torch.Size([1, 32, 64, 64])
